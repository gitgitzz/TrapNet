...
def calc_fid(recovery_img_path, private_img_path, batch_size=64, dims=2048):
    """
    ...
    inception_model = inception.InceptionV3([block_idx]).to(device)
    ...
    for _ in range(1):
        ...
        if flag:
            break
    print("num of reconstructed images for FID:{}".format(len(recovery_list)))
    ...
    mu_fake, sigma_fake = metrics.fid.calculate_activation_statistics(
        recovery_images, inception_model, batch_size, dims=dims, device=device
    )
    mu_real, sigma_real = metrics.fid.calculate_activation_statistics(
        private_images, inception_model, batch_size, dims=dims, device=device
    )
